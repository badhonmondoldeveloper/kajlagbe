...
export function CategoryDetailsView({ category, matchingProviders }: CategoryDetailsViewProps) {
  const [selectedSubService, setSelectedSubService] = React.useState<{
    name: string;
    priceRange: string;
  } | null>(null);
  const [isModalOpen, setIsModalOpen] = React.useState(false);
  const [selectedProviderName, setSelectedProviderName] = React.useState<string | undefined>(undefined);
  ...
  const handleOpenBooking = (subName?: string, priceRange?: string, providerName?: string) => {
    if (subName && priceRange) {
      setSelectedSubService({ name: subName, priceRange });
    } else {
  ...
  return (
    <div className="space-y-12 sm:space-y-16 pb-16">
      ...
      <QuickBookingModal
        ...
        serviceTitle={selectedSubService?.name || `${category.title} সেবা`}
        estimatedPrice={selectedSubService?.priceRange || `শুরু মাত্র ৳ ${category.startingPrice}`}
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
            ...
            <div className="w-full lg:w-96 rounded-2xl border border-emerald-200 bg-white p-6 shadow-md space-y-4">
              <div className="space-y-1">
                <h3 className="text-base font-bold text-slate-900">ইনস্ট্যান্ট সার্ভিস বুকিং</h3>
                <p className="text-xs text-slate-500">আপনার এলাকায় ১৫-৩০ মিনিটে টেকনিশিয়ান অর্ডার করুন</p>
              </div>
              ...
              <div className="flex items-center gap-2 text-[11px] text-slate-400 justify-center">
                ...
                <span>নিরাপদ ক্যাশ / বিকাশ / এসক্রো পেমেন্ট</span>
              </div>
            </div>
          </div>
        </Container>
            ...
            >
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                ...
                <Button
                  size="sm"
                  variant="primary"
                  onClick={() => handleOpenBooking(sub.name, sub.priceRange)}
                >
                  ⚡ কুইক বুকিং
                </Button>
              </div>
              ...
              <Card key={prov.id} variant="interactive" className="flex flex-col justify-between">
                ...
                <CardFooter className="flex items-center justify-between gap-2">
                  <Button
                    ...
                    variant="primary"
                    className="w-1/2"
                    onClick={() => handleOpenBooking(undefined, undefined, prov.name)}
                  >
                    ⚡ কুইক বুক
                  </Button>
                  <Link href={`/providers/${prov.slug}`} className="w-1/2">
